...
class User extends UFModel {
    ...
    public function checkAccess($hook, $params = []){
        ...
        $group_id =  $this->primary_group_id;
        ...
        if ($group_id == 2 || $this->id == static::$app->config('user_id_master'))  // Need to use loose comparison for now, because some DBs return `id` as a string
            return true;
