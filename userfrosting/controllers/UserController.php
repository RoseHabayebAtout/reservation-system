...
//            }
            ...
            if (!$paginate_server_side) {
                ...
                if ($this->_app->user->primary_group_id == 2) {
                    ...
                    $user_collection = User::where('id', $this->_app->user->id)->get();
                }
...
//        }
        ...
        foreach ($fields as $field){
           // if ($this->_app->user->checkAccess("view_account_setting", ["user" => $target_user, "property" => $field]))
            if ($this->_app->user->checkAccess("uri_users", ["user" => $target_user, "property" => $field]))
...
//        }
    ...
    //    if (!$this->_app->user->checkAccess("update_account_setting", ["user" => $target_user, "property" => 'password']))
        if (!$this->_app->user->checkAccess("uri_users", ["user" => $target_user, "property" => 'password']))
    ...
    public function createUser(){
        ...
        $primaryGroup = Group::where('id', $data['primary_group_id'])->first();;
    ...
    //    if (!isset($data['locale']) || !$this->_app->user->checkAccess("update_account_setting", ["property" => "locale"]))
        if (!isset($data['locale']) || !$this->_app->user->checkAccess("uri_users", ["property" => "locale"]))
            $data['locale'] = $this->_app->site->default_locale;

     //   if (!isset($data['title']) || !$this->_app->user->checkAccess("update_account_setting", ["property" => "title"])) {
        if (!isset($data['title']) || !$this->_app->user->checkAccess("uri_users", ["property" => "title"])) {
            // Set default title for new users
  ...
  //      if (!isset($data['primary_group_id']) || !$this->_app->user->checkAccess("update_account_setting", ["property" => "primary_group_id"])) {
        if (!isset($data['primary_group_id']) || !$this->_app->user->checkAccess("uri_users", ["property" => "primary_group_id"])) {
   ...
   //     if (!isset($data['groups']) || !$this->_app->user->checkAccess("update_account_setting", ["property" => "groups"])) {
        if (!isset($data['groups']) || !$this->_app->user->checkAccess("uri_users", ["property" => "groups"])) {
    ...
    public function updateUser($user_id){
        ...
        foreach ($post as $name => $value) {
            if ($name == "groups" || ($target_user->attributeExists($name) && $post[$name] != $target_user->$name)){
              ...
              //  if (!$this->_app->user->checkAccess('update_account_setting', ['user' => $target_user, 'property' => $name])){
                if (!$this->_app->user->checkAccess('uri_users', ['user' => $target_user, 'property' => $name])){
                    $ms->addMessageTranslated("danger", "ACCESS_DENIED");
                    $this->_app->halt(403);
                }
      ...
      //      if (!$this->_app->user->checkAccess('update_account_setting', ['user' => $target_user, 'property' => 'flag_password_reset'])){
            if (!$this->_app->user->checkAccess('uri_users', ['user' => $target_user, 'property' => 'flag_password_reset'])){
                $ms->addMessageTranslated("danger", "ACCESS_DENIED");
                $this->_app->halt(403);
            }
    ...
    public function deleteUser($user_id){
        ...
        // Check authorization
        if (!$this->_app->user->checkAccess('uri_users', ['user' => $target_user])){
            $ms->addMessageTranslated("danger", "ACCESS_DENIED");
            $this->_app->halt(403);
        }
    ...
    public function getTotalUsers(){
        $users = User::get()->where('primary_group_id', 1)->all();
...
$conn=sqlsrv_connect('SD-PC-W10-6048\SQLEXPRESS',$connectionInfo);
