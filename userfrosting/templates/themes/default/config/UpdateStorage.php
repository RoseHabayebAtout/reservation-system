...
if ($back_url == "") {
    $back_url = "http://$_SERVER[HTTP_HOST]/$urlInPieces[1]/public/uploadStorage/";
}
...
if (isset($_FILES['userfile1']) && $_FILES['userfile1']['size'] > 0) {
    ...
    $success_message = 'All storages have been updated successfully!';
    ...
    $right_title = ['Storage Code', 'Neighborhood', 'Tabu Code', 'Building',
        'Floor', 'Storage Number', 'Tabo Discription',
        'Area', 'Price'
    ];
...
//echo print_r($worksheet->getCell('I1')->getValue());
    ...
    for ($row = 2; $row <= $lastRow; $row++) {
        ...
        $x7 = trim($worksheet->getCell('H' . $row)->getValue());
        $x8 = trim($worksheet->getCell('I' . $row)->getValue());


       $query = "UPDATE `storages` SET `tabu_code`= '". $x2 ."' ,`building`= '". $x3 ."' ,`floor`= '". $x4 ."' ,`storage_number`= '". $x5 ."' ,`tabu_description`= '". $x6 ."' ,`area`= '". $x7 ."' 
                                                    ,`price`= '". $x8 ."'  WHERE `rawabi_code` =  '". $x0 ."' and `neighporhood`=  '". $x1 ."' ";
