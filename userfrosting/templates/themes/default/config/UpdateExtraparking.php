...
if (isset($_FILES['userfile1']) && $_FILES['userfile1']['size'] > 0) {
    ...
    $success_message = 'All extra parkings have been Updated successfully!';
...
//echo print_r($worksheet->getCell('I1')->getValue());
    ...
    for ($row = 2; $row <= $lastRow; $row++) {
        ...
        $query = "UPDATE `extra_parking` SET `building`='". $x2 ."',`floor`='". $x3 ."',`parking_number`='". $x4 ."',`description`='". $x5 ."',`price`='". $x6 ."' WHERE `neighporhood` = '". $x1 ."' and `rawabi_code` = '". $x0 ."' ";
